...
public class ResourceVersionMojo extends ResourceMojo {
	...
	private void iterateDir(File file) throws IOException {
		if (file.isDirectory()) {
			...
			if (StringUtils.isNotEmpty(sufix)
					&& ArrayUtils.contains(suffixs, sufix.toLowerCase())) {
	...
	public String resolveJsRefer(String page) {
		...
		while (true) {
			...
			pfull = pfull.substring(0, matcher.start(1) - matcher.start())
					+ resolveStaticAddr(jsdirPart.trim())
					+ pfull.charAt(pfull.length() - 1);
	...
	public String resolveCssRefer(String page) {
		...
		while (matcher.find()) {
			...
			pfull = pfull.substring(0, matcher.start(1) - matcher.start())
					+ resolveStaticAddr(cssPath.trim())
					+ pfull.charAt(pfull.length() - 1);
			matcher.appendReplacement(sb, Matcher.quoteReplacement(pfull));
		}
		matcher.appendTail(sb);
